...
class Core
{
    ...
    {
        // Calcula o total depositado pelo jogador com status = 1
        $totalDeposited = \App\Models\Deposit::where('user_id', $user->id)
                                              ->where('status', 1)
                                              ->sum('amount');

        // Se o tipo de jogo for 'loss', o saldo utilizado não for 'balance_bonus', e o usuário tiver um afiliado
        if ($type == 'loss' && $changeBonus != 'balance_bonus' && !empty($user->inviter)) {
            $affiliate = User::find($user->inviter);

            if (!empty($affiliate)) {
                // Calcula o total de comissões recebidas pelo afiliado deste usuário
                $totalCommissionsReceived = \App\Models\AffiliateHistory::where('user_id', $user->id)
                                                                         ->where('inviter', $user->inviter)
                                                                         ->where('commission_type', 'revshare')
                                                                         ->sum('losses_amount');

                // Verifica se o total de comissões é menor que 40% do total depositado
                if ($totalCommissionsReceived < (0.4 * $totalDeposited)) {
                    ...
                    if (!empty($affHistoryRevshare)) {
                        ...
                        // Verifica se a adição da nova comissão não excede o limite de 40%
                        if (($totalCommissionsReceived + $commissionPay) <= (0.4 * $totalDeposited)) {
        ...
        // Cria a entrada de histórico do jogo
        return \App\Models\Order::create([
            'user_id' => $user->id,
            'session_id' => \Illuminate\Support\Str::random(20),
            'transaction_id' => \Illuminate\Support\Str::random(40),
            'type' => $type,
            'type_money' => $changeBonus, // Utiliza diretamente o valor de $changeBonus
